...
*/
public class AssignToOneOrToAllImpl extends AbstractAssignToOneOrToAllImpl {
	...
	public List<Long> filter(final String actorName) throws UserFilterException {
		...
		Long userId = getUserToAssign();
		if(userId > 0) {
			return Arrays.asList(userId);
		}else {
			return getAPIAccessor().getProcessAPI().getUserIdsForActor(getExecutionContext().getProcessDefinitionId(), actorName, 0, Integer.MAX_VALUE);
		}
